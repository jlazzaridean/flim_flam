        ...
        if(contains(thisVal,'overlay'))
            ...
            if(strcmp(data(i,1).binType,'std'))
                if(strcmp(ccvName,'tau1') || strcmp(ccvName,'tau2') || strcmp(ccvName,'tau3') || strcmp(ccvName,'tau4'))
                    ...
                    [oFig, f2, oImg] = renderOverlay(data(i,1).photonsBinned,data(i,1).tau(:,:,tNum),photonsMax,varargin{1,overlayCount},oName);
                elseif (strcmp(ccvName,'a1') || strcmp(ccvName,'a2') || strcmp(ccvName,'a3') || strcmp(ccvName,'a4'))
                    aNum = str2double(ccvName(end));
                    [oFig, f2, oImg] = renderOverlay(data(i,1).photonsBinned,data(i,1).a(:,:,aNum),photonsMax,varargin{1,overlayCount},oName);
                else
                    [oFig, f2, oImg] = renderOverlay(data(i,1).photonsBinned,data(i,1).(ccvName),photonsMax,varargin{1,overlayCount},oName);
                end
            elseif(strcmp(data(i,1).binType,'bh'))
                if(strcmp(ccvName,'tau1') || strcmp(ccvName,'tau2') || strcmp(ccvName,'tau3') || strcmp(ccvName,'tau4'))
                    tNum = str2double(ccvName(end));
                    [oFig, f2, oImg] = renderOverlay(data(i,1).photons,data(i,1).tau(:,:,tNum),photonsMax,varargin{1,overlayCount},oName);
                elseif (strcmp(ccvName,'a1') || strcmp(ccvName,'a2') || strcmp(ccvName,'a3') || strcmp(ccvName,'a4'))
                    aNum = str2double(ccvName(end));
                    [oFig, f2, oImg] = renderOverlay(data(i,1).photons,data(i,1).a(:,:,aNum),photonsMax,varargin{1,overlayCount},oName);
                else
                    [oFig, f2, oImg] = renderOverlay(data(i,1).photons,data(i,1).(ccvName),photonsMax,varargin{1,overlayCount},oName);
                end                
            else
