...
%convenientPath = 'C:\Users\JLD\PhD\Box Sync\2019 Aniline\00 characterization\FLIM-electrophysiology\RawDatasets\';
convenientPath = 'C:\Users\JLD\PhD\ImagingData\Chang880Data\';
fsep = filesep;
...
for i=size(irfStruct,1)
    if(size(irfStruct(i,1).IRF,1) ~= ADCres)
        disp('Some of the selected IRFs contain a number of ADC channels inconsistent with the config specifications.');
        disp('Padding end with zeros.');
        irfStruct(i,1).IRF(end:ADCres) = 0;
    end
end
...
for i=1:nFiles
    ...
    elseif(strcmp(importMode,'pxwise'))
        ...
        %mmuch else
        writeTIFF(sum(data(i,1).tcspc,3),'analysis',photonPath,strcat(data(i,1).fName(1:end-4),'_rawPhotons'));
    elseif(contains(importMode,'global'))
        ...
        writeTIFF(photons,'analysis',photonPath,strcat(data(i,1).fName(1:end-4),'_rawPhotons'));
        if(contains(importMode,'btm'))
            ...
            for j = 1:nROIs
                writeTIFF(data(i,1).masks(:,:,j),'analysis',maskPath,strcat(data(i,1).fName(1:end-4),'_mask',num2str(j)));
